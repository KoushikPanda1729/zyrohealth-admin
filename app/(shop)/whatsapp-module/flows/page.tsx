...
export default function ShopWhatsAppFlowsPage() {
  ...
  const fetchFlows = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await apiCall('GET', '/api/shop/whatsapp-module/flows');
  ...
  const createFlow = async () => {
    ...
    try {
      const result = await apiCall('POST', '/api/shop/whatsapp-module/flows', { name: newName.trim() });
      const flow = result.data ?? result;
      setCreating(false);
      setNewName('');
      router.push(`/whatsapp-module/flows/${flow.id}`);
    } catch (err: unknown) {
  ...
  const generateFlow = async () => {
    ...
    setGenLoading(true);
    try {
      const result = await apiCall('POST', '/api/shop/whatsapp-module/flows/generate', {
        name: genName.trim(),
  ...
  const toggleActive = async (flow: WhatsAppFlow) => {
    setBusyId(flow.id);
    try {
      await apiCall('PATCH', `/api/shop/whatsapp-module/flows/${flow.id}/${flow.isActive ? 'deactivate' : 'activate'}`);
      message.success(flow.isActive ? 'Flow deactivated' : 'Flow activated — it now handles all incoming messages to your module number');
      fetchFlows();
    } catch (err: unknown) {
  ...
  const deleteFlow = async (id: string) => {
    setBusyId(id);
    try {
      await apiCall('DELETE', `/api/shop/whatsapp-module/flows/${id}`);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: unknown, f: WhatsAppFlow) => (
        <Space>
          <Button size="small" icon={<EditOutlined />} onClick={() => router.push(`/whatsapp-module/flows/${f.id}`)}>Edit</Button>
          <Button
            size="small"
    ...
    <div>
      ...
      {flows.length === 0 && !loading && !error && (
        <Alert
          ...
          description="Create a flow to define how your WhatsApp module responds to customers. Only one flow can be active at a time — while active, it drives every incoming message to your module number. With no active flow, the number simply won't respond."
      ...
      {loading ? (
        ...
        <Table columns={columns} dataSource={flows.map((f) => ({ ...f, key: f.id }))} bordered size="middle" scroll={{ x: 'max-content' }} />
      )}
      ...
      >
        <Input
          placeholder="Flow name (e.g. Welcome Menu)"
          value={newName}
      ...
      >
        <Space direction="vertical" style={{ width: '100%' }}>
          <div>
            ...
            <Input
              placeholder="e.g. Order Medicine"
              value={genName}
          ...
          <div>
            ...
            <TextArea
              rows={5}
              placeholder='e.g. "Greet the customer, ask them to send a photo of their prescription, then tell them we will reply with a price."'
              value={genPrompt}
