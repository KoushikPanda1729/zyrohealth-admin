...
export default function AccountPage() {
  ...
  // The admin layout's header/sidebar show the user's name+avatar from a
  // snapshot read out of localStorage once at mount — they have no way to
  // know this page just changed it. Update storage AND broadcast a custom
  // event so the layout (listening in its own effect) can refresh live,
  // without needing a full page reload to see the new photo/name.
  ...
  const generateBio = async () => {
    ...
    try {
      ...
      const result = await apiCall('POST', '/api/auth/me/generate-bio', {
        ...
        roleLabel: me ? (ROLE_LABELS[me.role] ?? me.role) : undefined,
        bio: current.bio,
      });
